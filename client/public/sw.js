...
self.addEventListener('fetch', (event) => {
  ...
  // Skip Vite dev-server paths. These are dynamically generated by Vite and
  // MUST go straight to the dev server — caching them is meaningless and
  // intercepting them turns "Vite isn't running" into a misleading 503 from
  // the catch-all at the bottom of this handler. By returning without calling
  // event.respondWith, the browser falls back to the default fetch, which
  // surfaces the real network error (ERR_CONNECTION_REFUSED) when Vite is
  // down so the developer can act on it. Has no effect in production where
  // these paths simply don't exist.
  if (
    ...
    url.search.includes('?t=') || url.search.includes('&t=')  // HMR cache-bust query
  ) {
    return;
  }
