...
export default function PersonCell({ value, owners = [], members = [], onChange, onOwnersChange }) {
  ...
  // Determine primary owner (first in owners array or the one with isPrimary)
  const primaryOwner = hasOwners
    ? owners.find(o => o.TaskOwner?.isPrimary) || owners[0]
    : null;

  const displayOwners = hasOwners ? owners.slice(0, 3) : [];
  const extraCount = hasOwners ? Math.max(0, owners.length - 3) : 0;
  ...
  // Render stacked avatars for multi-owner display
  if (hasOwners && !open) {
    return (
        ...
        <button ref={btnRef} onClick={(e) => { e.stopPropagation(); if (!onChange && !onOwnersChange) return; setOpen(!open); setSearch(''); }}
          ...
          <div className="flex items-center -space-x-1.5">
            {displayOwners.map((owner, idx) => (
              <div key={owner.id} className="relative" style={{ zIndex: displayOwners.length - idx }}>
                <Avatar name={owner.name} image={owner.avatar ? `/uploads/avatars/${owner.avatar}` : undefined} size="xs" />
                {primaryOwner && primaryOwner.id === owner.id && (
