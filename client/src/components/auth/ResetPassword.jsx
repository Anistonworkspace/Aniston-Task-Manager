...
export default function ResetPassword() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      await api.post('/auth/reset-password', { token, newPassword: password, confirmNewPassword: confirm });
      setDone(true);
    } catch (err) {
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
                      className="w-full pl-10 pr-10 py-2.5 rounded-lg border border-border text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" placeholder="Min 8 characters" />
