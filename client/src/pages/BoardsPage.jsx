...
export default function BoardsPage() {
  ...
  const { user, isSuperAdmin, permissionGrants, effectivePermissions } = useAuth();
  const canCreateBoard = canUser(user?.role, 'create_board', isSuperAdmin, permissionGrants, effectivePermissions);
    ...
    <div className="p-6">
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          {!searchQuery && canCreateBoard && <button onClick={() => setShowCreate(true)} className="px-4 py-2 bg-primary text-white text-sm rounded-md font-medium hover:bg-primary-hover"><Plus size={14} className="inline mr-1" /> Create Board</button>}
        </div>
      ) : (
