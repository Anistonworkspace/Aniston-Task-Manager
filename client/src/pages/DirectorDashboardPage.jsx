...
export default function DirectorDashboardPage() {
  ...
  const loadPlan = useCallback(async (dirId) => {
    try {
      const resolvedId = dirId !== undefined ? dirId : selectedDirectorIdRef.current;
      const dirParam = resolvedId ? `?directorId=${resolvedId}` : '';
  ...
  const loadOrgDataRef = useRef(loadOrgData);
  const loadPlanRef = useRef(loadPlan);
  useEffect(() => { loadOrgDataRef.current = loadOrgData; }, [loadOrgData]);
  useEffect(() => { loadPlanRef.current = loadPlan; }, [loadPlan]);

  // Single init effect — only re-runs if canAccess changes
  useEffect(() => {
    if (!canAccess) { setLoading(false); return; }
    let cancelled = false;
    setLoading(true);
    // Safety: force-clear skeleton after 5s even if API hangs
    loadTimerRef.current = setTimeout(() => { if (!cancelled) setLoading(false); }, 5000);
    (async () => {
      try {
        const dirsRes = await api.get('/director-plan/directors');
        if (cancelled) return;
        ...
        if (dirs.length > 0) {
          ...
          dirId = self ? self.id : dirs[0].id;
          setSelectedDirectorId(dirId);
          selectedDirectorIdRef.current = dirId;
        }
        await Promise.all([loadOrgDataRef.current(), loadPlanRef.current(dirId)]);
      } catch (e) {
        console.error('Failed to init director dashboard:', e);
      } finally {
        if (!cancelled) { clearTimeout(loadTimerRef.current); setLoading(false); }
      }
    })();
    const orgInterval = setInterval(() => { if (!cancelled) loadOrgDataRef.current(); }, 5 * 60 * 1000);
    return () => { cancelled = true; clearTimeout(loadTimerRef.current); clearInterval(orgInterval); };
  }, [canAccess]); // eslint-disable-line react-hooks/exhaustive-deps

  // Reload plan when director selection changes after initial load
  useEffect(() => {
    if (selectedDirectorId) loadPlanRef.current(selectedDirectorId);
  }, [selectedDirectorId]); // eslint-disable-line react-hooks/exhaustive-deps
